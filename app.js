...
async function handleTemplateUpload(event) {
    ...
    try {
        ...
        alert('Error reading template file. Please make sure it\'s a valid .docx file.');
    }
}
...
function displayExcelPreview(data) {
    if (data.length === 0) {
        excelPreview.innerHTML = '<p style="color: #ef4444;">No data found in Excel file.</p>';
    ...
    if (data.length > 5) {
        html += `<p style="margin-top: 12px; color: #6b7280; font-size: 0.9rem;">... and ${data.length - 5} more rows</p>`;
    }
...
async function generateCertificates() {
    ...
    try {
        for (let i = 0; i < studentData.length; i++) {
            ...
            progressText.textContent = `Processing ${i + 1} of ${totalStudents} certificates...`;
        ...
        resultsSummary.textContent = `Successfully generated ${successCount} certificate(s) in ${format === 'both' ? 'Word and PDF' : format.toUpperCase()} format!`;
...
async function generateDocxCertificate(studentData) {
    try {
        ...
        // Download file
        const fileName = `${sanitizeFileName(studentData.name || 'certificate')}_certificate.docx`;
        saveAs(blob, fileName);
        
    } catch (error) {
        console.error('Error generating DOCX:', error);
        throw new Error(`Failed to generate DOCX for ${studentData.name}: ${error.message}`);
    }
}
...
async function generatePdfCertificate(studentData) {
    try {
        ...
        tempDiv.innerHTML = `
            <div style="border: 8px solid #4f46e5; padding: 40px; text-align: center;">
                ...
                <div style="margin: 40px 0; font-size: 18px; line-height: 1.8;">
                    ...
                    <h2 style="font-size: 36px; margin: 20px 0; color: #1f2937;">${studentData.name || 'Student Name'}</h2>
                    <p>has successfully completed</p>
                    <h3 style="font-size: 28px; margin: 20px 0; color: #1f2937;">${studentData.course || 'Course Name'}</h3>
                    ${studentData.grade ? `<p>with a grade of <strong>${studentData.grade}</strong></p>` : ''}
                    ${studentData.date ? `<p style="margin-top: 30px;">Date: ${studentData.date}</p>` : ''}
                </div>
            </div>
        `;
        ...
        // Download file
        const fileName = `${sanitizeFileName(studentData.name || 'certificate')}_certificate.pdf`;
        pdf.save(fileName);
        
    } catch (error) {
        console.error('Error generating PDF:', error);
        throw new Error(`Failed to generate PDF for ${studentData.name}: ${error.message}`);
    }
}
